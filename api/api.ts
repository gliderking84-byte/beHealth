 ...
 * Skills (system prompts) are injected via getSystemPrompt() from skills.ts.
 */
...
// Prefer getSystemPrompt() + buildPatientContext() from skills.ts for new code.
...
export function buildHealthContext(profile: HealthProfile, latestBalance?: BalanceEntry): string {
  ...
  const balance = latestBalance
    ? `Sleep: ${latestBalance.sleep}h, Work: ${latestBalance.work}h, Stress: ${latestBalance.stress}/10, Balance: ${latestBalance.balanceScore}/100`
    : ''

  return `Patient: ${profile.name}, ${profile.age}yo ${profile.sex}. Health score: ${profile.healthScore}/100. Labs: ${labs}.${balance ? ' ' + balance : ''}`
}
