...
export default function SpinePage() {
  ...
  // ── Export Clinical Folder PDF ────────────────────────────────────────────
  function exportClinicalFolder() {
    import('jspdf').then(({ default: jsPDF }) => {
      const doc   = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
      const pageW = doc.internal.pageSize.getWidth()
      const pageH = doc.internal.pageSize.getHeight()
      ...
      doc.text(isIt ? 'Cartella Clinica Ortopedica' : 'Orthopedic Clinical Folder', m + 34, 14)
      doc.text(new Date().toLocaleDateString(isIt ? 'it-IT' : 'en-GB'), pageW - m, 14, { align: 'right' })
      y = 36

      // Patient info
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...DARK)
      doc.text(isIt ? 'Paziente' : 'Patient', m, y)
      doc.setFont('helvetica', 'normal'); doc.setFontSize(9)
      doc.text(`${profile.name}  ·  ${profile.age ?? '—'} ${isIt ? 'anni' : 'years'}  ·  ${profile.sex ?? '—'}`, m, y + 5)
      y += 14
      ...
      doc.text(isIt ? 'STORICO SESSIONI' : 'SESSION HISTORY', m + 3, y + 4.8)
      y += 10

      spineSessions.forEach((s, i) => {
        checkPage(8)
        const d = new Date(s.date).toLocaleDateString(isIt ? 'it-IT' : 'en-GB', { day: '2-digit', month: '2-digit', year: '2-digit' })
        const urgMap: Record<string, string> = { URGENTE: '🔴', SIGNIFICATIVO: '🟠', MODERATO: '🟡', LIEVE: '🟢' }
        doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(...DARK)
        doc.text(`${i + 1}. ${d}  ${urgMap[s.urgency] ?? '•'} ${s.urgency}  —  ${s.fileName}`, m + 3, y)
        if (s.summary) {
          doc.setTextColor(...GRAY); doc.setFontSize(7)
          const lines = doc.splitTextToSize(`    ${s.summary}`, cW - 6)
      ...
      if (analysis) {
        const addSec = (title: string, text: string) => {
          ...
          doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(...DARK)
          const lines = doc.splitTextToSize(text.replace(/[*#]/g, '').trim(), cW)
          lines.forEach((line: string) => { checkPage(5); doc.text(line, m, y); y += 4.2 })
          y += 4
        }
        ...
        doc.text(isIt ? 'ULTIMA ANALISI COMPLETA' : 'LATEST FULL ANALYSIS', m + 3, y + 5.5)
        y += 12

        addSec(isIt ? 'Quadro Clinico' : 'Clinical Picture', analysis.quadro)
        addSec(isIt ? 'Interpretazione Imaging' : 'Imaging', analysis.imaging)
        addSec(isIt ? 'Diagnosi' : 'Diagnosis', analysis.diagnosi)
        if (analysis.redFlags) addSec(isIt ? 'Red Flags' : 'Red Flags', analysis.redFlags)
        addSec(isIt ? 'Piano di Gestione' : 'Management Plan', analysis.piano)
        addSec(isIt ? 'Protocollo Riabilitativo' : 'Rehabilitation', analysis.riabilitazione)
        addSec(isIt ? 'Esami Raccomandati' : 'Tests', analysis.esami)
      }
      ...
      for (let p = 1; p <= pages; p++) {
        ...
        doc.text(isIt ? 'Generato da BeHealth AI — non sostituisce la visita specialistica' : 'Generated by BeHealth AI — does not replace specialist consultation', m, pageH - 5)
        doc.text(`${p} / ${pages}`, pageW - m, pageH - 5, { align: 'right' })
      }

      const dateStr = new Date().toLocaleDateString('it-IT').replace(/\//g, '-')
      doc.save(`cartella-ortopedica-${dateStr}.pdf`)
    })
  }
